import { EnvelopeIcon, MapIcon, PhoneIcon } from "@heroicons/react/16/solid";
import React from "react";

const Footer = () => {
  const emailAddress = "deversifytech@gmail.com";
  const phoneNumber = "+917988064796";

  const scrollToSection = (sectionId: string) => {
    const section = document.getElementById(sectionId);
    if (section) {
      section.scrollIntoView({ behavior: "smooth" });
    }
  };

  return (
    <footer className="pt-[5rem] pb-[3rem] bg-black">
      <div className="w-[80%] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[2rem] border-b-[1.4px] pb-5 border-gray-600 border-opacity-40">
        <div>
          <div className="font-logo text-white text-[18px]">
            <span className="text-[30px] md:text-[40px] text-yellow-300">
              Deversify
            </span>{" "}
            Tech
          </div>
          <h1 className="text-[14px] mt-[0.5rem] text-white opacity-70 mb-3">
            Crafting effective websites tailored to your business needs. With
            years of industry experience, we deliver top-notch solutions to help
            your brand stand out digitally
          </h1>
          <a
            href={`mailto:${emailAddress}`}
            className=" text-yellow-300 underline font-semibold"
          >
            {emailAddress}
          </a>
        </div>
        <div className="md:mx-auto">
          <h1 className="text-white font-semibold mb-[1.4rem] text-[17px]">
            Quick Link
          </h1>
          <p
            className="text-[15px] text-white opacity-80 mb-[1rem] cursor-pointer hover:text-yellow-300"
            onClick={() => scrollToSection("about-us")}
          >
            About Us
          </p>
          <p
            className="text-[15px] text-white opacity-80 mb-[1rem] cursor-pointer hover:text-yellow-300"
            onClick={() => scrollToSection("services")}
          >
            Our Services
          </p>
          <p
            className="text-[15px] text-white opacity-80 mb-[1rem] cursor-pointer hover:text-yellow-300"
            onClick={() => scrollToSection("work")}
          >
            Our Work
          </p>
          <p
            className="text-[15px] text-white opacity-80 mb-[1rem] cursor-pointer hover:text-yellow-300"
            onClick={() => scrollToSection("contact")}
          >
            Contact Us
          </p>
        </div>
        <div className="lg:mx-auto">
          <h1 className="text-white font-semibold mb-[1.4rem] text-[17px]">
            Address
          </h1>
          <div className="flex items-center mt-[1rem] space-x-2">
            <MapIcon className="w-[1rem] h-[1rem] text-yellow-300" />
            <p className="text-[17px] font-normal text-white opacity-75">
              Kalka , Haryana
            </p>
          </div>
          <div className="flex items-center mt-[1rem] space-x-2">
            <EnvelopeIcon className="w-[1rem] h-[1rem] text-yellow-300" />
            <a
              href={`mailto:${emailAddress}`}
              className="text-[17px] font-normal text-white opacity-75"
            >
              {emailAddress}
            </a>
          </div>
          <div className="flex items-center mt-[1rem] space-x-2">
            <PhoneIcon className="w-[1rem] h-[1rem] text-yellow-300" />
            <a
              href={`tel:${phoneNumber}`}
              className="text-[17px] font-normal text-white opacity-75"
            >
              {phoneNumber}
            </a>
          </div>
        </div>
      </div>
      <div className="mt-[1.4rem] w-[80%] mx-auto text-white opacity-70 flex justify-between items-center">
        &#169; Copyright deversify tech {new Date().getFullYear()}
        <div className="flex gap-6">
          <a
            href="https://www.instagram.com/deversifytech?igshid=MXNiNXplZ2Y2MnpmbA=="
            className="hover:text-yellow-300"
          >
            Instagram
          </a>
          <a
            href="https://www.linkedin.com/company/deversfiy-tech/"
            className="hover:text-yellow-300"
          >
            Linkedin
          </a>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
